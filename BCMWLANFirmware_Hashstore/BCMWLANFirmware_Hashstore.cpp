...
*/
#include "BCMWLANFirmware_Hashstore.hpp"
...
OSDefineMetaClassAndStructors(BCMWLANFirmware_Hashstore, IO80211Controller);
OSDefineMetaClassAndStructors(CTimeout, OSObject)

IO80211WorkLoop *_fWorkloop;
IOCommandGate *_fCommandGate;

bool BCMWLANFirmware_Hashstore::init(OSDictionary *properties)
{
...
IOService* BCMWLANFirmware_Hashstore::probe(IOService *provider, SInt32 *score)
{
    bool isMatch = false;
...
bool BCMWLANFirmware_Hashstore::configureInterface(IONetworkInterface *netif) {
    IONetworkData *nd;
...
IONetworkInterface *BCMWLANFirmware_Hashstore::createInterface()
{
    BCMWLANFirmware_HashstoreInterface *netif = new BCMWLANFirmware_HashstoreInterface;
    if (!netif) {
        return NULL;
    }
...
void BCMWLANFirmware_Hashstore::associateSSID(uint8_t *ssid, uint32_t ssid_len, const struct ether_addr &bssid, uint32_t authtype_lower, uint32_t authtype_upper, uint8_t *key, uint32_t key_len, int key_index) 
{
...
void BCMWLANFirmware_Hashstore::setPTK(const u_int8_t *key, size_t key_len) {
    struct ieee80211com *ic = fHalService->get80211Controller();
...
void BCMWLANFirmware_Hashstore::setGTK(const u_int8_t *gtk, size_t key_len, u_int8_t kid, u_int8_t *rsc) {
    struct ieee80211com *ic = fHalService->get80211Controller();
    struct ieee80211_node	* ni = ic->ic_bss;
    struct ieee80211_key *k;
    int keylen;
...
bool BCMWLANFirmware_Hashstore::
...
bool BCMWLANFirmware_Hashstore::start(IOService *provider)
{
    ...
    _fCommandGate = IOCommandGate::commandGate(this, (IOCommandGate::Action)BCMWLANFirmware_Hashstore::tsleepHandler);
    if (_fCommandGate == 0) {
    ...
    watchdogTimer = IOTimerEventSource::timerEventSource(this, OSMemberFunctionCast(IOTimerEventSource::Action, this, &BCMWLANFirmware_Hashstore::watchdogAction));
    if (!watchdogTimer) {
...
void BCMWLANFirmware_Hashstore::watchdogAction(IOTimerEventSource *timer)
{
...
void BCMWLANFirmware_Hashstore::fakeScanDone(OSObject *owner, IOTimerEventSource *sender)
{
    BCMWLANFirmware_Hashstore *that = (BCMWLANFirmware_Hashstore *)owner;
    that->getNetworkInterface()->postMessage(APPLE80211_M_SCAN_DONE);
}

const OSString * BCMWLANFirmware_Hashstore::newVendorString() const
{
    return OSString::withCString("Apple");
}

const OSString * BCMWLANFirmware_Hashstore::newModelString() const
{
    return OSString::withCString(fHalService->getDriverInfo()->getFirmwareName());
}

bool BCMWLANFirmware_Hashstore::initPCIPowerManagment(IOPCIDevice *provider)
{
    UInt16 reg16;
...
IOReturn BCMWLANFirmware_Hashstore::selectMedium(const IONetworkMedium *medium)
{
    setSelectedMedium(medium);
    return kIOReturnSuccess;
}

void BCMWLANFirmware_Hashstore::stop(IOService *provider)
{
    XYLog("%s\n", __FUNCTION__);
    struct _ifnet *ifp = &fHalService->get80211Controller()->ic_ac.ac_if;
    super::stop(provider);
...
IOReturn BCMWLANFirmware_Hashstore::
setLinkStateGated(OSObject *target, void *arg0, void *arg1, void *arg2, void *arg3)
{
    BCMWLANFirmware_Hashstore *that = OSDynamicCast(BCMWLANFirmware_Hashstore, target);
...
void BCMWLANFirmware_Hashstore::releaseAll()
{
    if (fHalService) {
        fHalService->release();
        fHalService = NULL;
...
void BCMWLANFirmware_Hashstore::free()
{
    XYLog("%s\n", __FUNCTION__);
    if (fHalService != NULL) {
        fHalService->release();
...
IOReturn BCMWLANFirmware_Hashstore::enable(IONetworkInterface *netif)
{
    XYLog("%s\n", __PRETTY_FUNCTION__);
    super::enable(netif);
...
IOReturn BCMWLANFirmware_Hashstore::disable(IONetworkInterface *netif)
{
    XYLog("%s\n", __PRETTY_FUNCTION__);
    super::disable(netif);
    setLinkStatus(kIONetworkLinkValid);
...
IOReturn BCMWLANFirmware_Hashstore::enableAdapter(IONetworkInterface *netif)
{
...
void BCMWLANFirmware_Hashstore::disableAdapter(IONetworkInterface *netif)
{
    watchdogTimer->cancelTimeout();
...
IOReturn BCMWLANFirmware_Hashstore::getHardwareAddress(IOEthernetAddress *addrP)
{
...
IOReturn BCMWLANFirmware_Hashstore::getHardwareAddressForInterface(
...
#ifdef __PRIVATE_SPI__
IOReturn BCMWLANFirmware_Hashstore::outputStart(IONetworkInterface *interface, IOOptionBits options)
{
    struct _ifnet *ifp = &fHalService->get80211Controller()->ic_ac.ac_if;
    mbuf_t m = NULL;
...
UInt32 BCMWLANFirmware_Hashstore::outputPacket(mbuf_t m, void *param)
{
//    XYLog("%s\n", __FUNCTION__);
...
UInt32 BCMWLANFirmware_Hashstore::getFeatures() const
{
    return fHalService->getDriverInfo()->supportedFeatures();
}

IOReturn BCMWLANFirmware_Hashstore::setPromiscuousMode(IOEnetPromiscuousMode mode)
{
    return kIOReturnSuccess;
}

IOReturn BCMWLANFirmware_Hashstore::setMulticastMode(IOEnetMulticastMode mode)
{
    return kIOReturnSuccess;
}

IOReturn BCMWLANFirmware_Hashstore::setMulticastList(IOEthernetAddress* addr, UInt32 len)
{
    return fHalService->getDriverController()->setMulticastList(addr, len);
}

SInt32 BCMWLANFirmware_Hashstore::monitorModeSetEnabled(
...
IOReturn BCMWLANFirmware_Hashstore::getPacketFilters(const OSSymbol *group, UInt32 *filters) const
{
...
{
    BCMWLANFirmware_Hashstore* dev = OSDynamicCast(BCMWLANFirmware_Hashstore, owner);
    if (dev == 0)
        return kIOReturnError;
...
void BCMWLANFirmware_Hashstore::unregistPM()
{
...
IOReturn BCMWLANFirmware_Hashstore::setPowerState(unsigned long powerStateOrdinal, IOService *policyMaker)
{
...
IOReturn BCMWLANFirmware_Hashstore::setWakeOnMagicPacket(bool active)
{
...
{
    BCMWLANFirmware_Hashstore *self = (BCMWLANFirmware_Hashstore *)param0;
...
IOReturn BCMWLANFirmware_Hashstore::registerWithPolicyMaker(IOService *policyMaker)
{
    IOReturn ret;
...
void BCMWLANFirmware_Hashstore::setPowerStateOff()
{
    XYLog("%s\n", __FUNCTION__);
...
void BCMWLANFirmware_Hashstore::setPowerStateOn()
{
    XYLog("%s\n", __FUNCTION__);
    pmPowerState = kPowerStateOn;
    pmPolicyMaker->acknowledgeSetPowerState();
...
UInt32 BCMWLANFirmware_Hashstore::
hardwareOutputQueueDepth(IO80211Interface *interface)
{
    return 0;
}

SInt32 BCMWLANFirmware_Hashstore::
...
IO80211VirtualInterface *BCMWLANFirmware_Hashstore::
...
void BCMWLANFirmware_Hashstore::
